...
// Ensure onCreateAccount is passed as a prop
const Login = ({ show, onHide, onCreateAccount, onForgotPassword }) => {
  const [formData, setFormData] = useState({
    email: '',
    password: ''
          ...
          <Col xs={12} sm={6}>
            <div className="d-flex justify-content-sm-end align-items-center gap-2">
              <p className="fs-6 mb-0">
                ...
                <Button
                  variant="link" // Makes it look like a link but acts as a button
                  className="p-0 text-decoration-none" // Remove default link padding/underline
                  onClick={(e) => {
                    e.preventDefault(); // Still good practice to prevent default if link-like
                    onCreateAccount && onCreateAccount(); // Call the prop to open Signup
                  }}
                >
                  Create an Account
                </Button>
